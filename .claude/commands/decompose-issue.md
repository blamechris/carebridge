...
- **Touches 3+ packages/services / multiple test surfaces / cross-cutting concerns** → good decomposition candidate.
...
- Identify the natural seams — places where the work splits into independently shippable pieces (different packages/services, different test surfaces, different acceptance criteria).
...
|---|----------------|-------|------------|
| 1 | type(scope): … | Packages: @carebridge/shared-types, @carebridge/validators | low |
| 2 | type(scope): … | Service: api-gateway tRPC router | medium |
| 3 | type(scope): … | Service: ai-oversight worker + clinical events | medium |

**Labels each sub-issue will receive:** enhancement, bug, or from-review (as applicable)
...
```bash
# Default sub-issue labels — use enhancement, bug, or from-review as appropriate
