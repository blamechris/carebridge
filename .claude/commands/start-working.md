...
| Ready | Unblocked, unassigned `enhancement` or `bug` issues; or `from-review` issues |
...
- `node_modules/`, `.next/`, `build/`, `dist/`, vendor directories
...
| P1 | High | `from-review` issues, unblocked unassigned `enhancement` or `bug` issues, stale PRs |
...
|--------|-------|---------------|
| TODO | 12 | services/ai-oversight/ (5), packages/validators/ (4), apps/clinician-portal/ (3) |
| FIXME | 3 | services/clinical-events/worker.ts (2), packages/medical-logic/index.ts (1) |
| HACK | 1 | services/api-gateway/routes.ts:42 |
```
...
- **If you want to clean up:** The {N} TODOs in services/ai-oversight/ suggest a refactoring pass
```
...
| Area | Status | Finding |
|------|--------|---------|
| Type Safety | ⚠️ | {N} source files have type errors |
...
1. **Fix type errors in {module}** — {N} files in `services/{path}/` have strict violations. Effort: M
