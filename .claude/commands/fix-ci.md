...
- `TypeScript strict violation` / `TS2322` / `TS2345` → FIX (type safety is enforced; fix the type error)
- `Missing .js extension in ESM import` / `Cannot find module` (ESM context) → FIX (add `.js` extension to relative imports)
- `Drizzle schema drift` / `migration pending` → FIX (run `pnpm db:migrate` and commit migration file)
- `Zod validation failed at boundary` → FIX (check `@carebridge/validators` schema matches input)
- `BullMQ worker crashed` / `queue connection failed` → RETRIGGER (transient Redis/queue issue)
