...
The resolved config is parsed + cached once at first access (#1112) — env changes after boot do not take effect without restart. In tests, `resetFanoutConfigCacheForTests()` clears the cache, and `loadFanoutConfig(env)` parses an explicit env map without touching the cache.
